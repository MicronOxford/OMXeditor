...
#               id = viewerID
        ...
        for w in [self.frame] + list(self.frame.GetChildren()):
...
def shellMenuAppendNewCommand(cmd, menuText="new command", menuCtrlKey='D', menu=0, bell=True, execModule=None):
    ...
    if bell:
       let bell sounf before calling f()
...
def vAnimate(vids=-1):
    ...
    def _animateStartStop(self):
        if self._animationRunning:
            self._animationRunning = False
        else:
            self._animateStart()
    def _animateStart(self):
        import time
        nn = 0
        t0 = time.time()
        self._animationRunning = True
        _animationZ = 0
        while self._animationRunning:
            try:
                vids = map(int, self._animateVids.split())
                axes = map(int, self._animateAxes.split())
                if len(axes) < len(vids):
                    axes += axes[-1:]*(len(vids)-len(axes))
                strides = map(int, self._animateStrides.split())
                if len(strides) < len(vids):
                    strides += strides[-1:]*(len(vids)-len(strides))
                #autoscales = map(int, self._animateAutoscale.split())
                ...
                for i,vid in enumerate(vids):
                    ...
                    z  = z+strides[i]*self.direction
                    zMax = vd(vid).shape[zaxis] -1 
                    if z > zMax:
                        z=0
                    elif z<0:
                        z = zMax
            ...
            if dt > 1:
                ...
                t0 = time.time()
            try:
                _animateMS = float(self._animateMS) / 1000.
                if _animateMS>0:
                    time.sleep( _animateMS )
            except:
                pass
    ...
    #global _animateObj
    #_animateObj = _animateClass()
    #global gp

    gp = guiParams()
    gp._animationRunning = False
    import new
    gp._animateStartStop = new.instancemethod(_animateStartStop, gp, guiParams)
    gp._animateStart = new.instancemethod(_animateStart, gp, guiParams)
    ...
    buttonBox([
    ("l\tvids"),
    ("t\t%s"%(vidsStr,), "_._animateVids = x"),
    '\n',
    ("l\taxis:"),
    ("t\t0", "_._animateAxes = x"),
    '\n',
    ("l\tstride:"),
    ("t\t1", "_._animateStrides = x"),
#    '\n',
#    ("l\tpause[ms]:"),
#    ("t\t0", "_._animateMS = x"),
    '\n',
    ...
    ("t _.fpsTxtCtrl = x\t--", "--"),
    '\n',] +
    gp._bboxInt('pause[ms]: ', '_animateMS', v=0, slider=True, slmin=0, slmax=1000, newLine=True)+[
    #("l\tautoscale:"),
    #("t\t0", "_._animateAutoscale = x"),
    #'\n',
    ("tb\tstart / stop", "_._animateStartStop()"),
    ("tb\treverse", "_.direction*=-1", .1),
    #], title='vAnimate', execModule=_animateObj)
